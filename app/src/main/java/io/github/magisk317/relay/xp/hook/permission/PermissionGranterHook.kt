...
class PermissionGranterHook : BaseHook() {
    ...
    override fun onLoadPackage(lpparam: XC_LoadPackage.LoadPackageParam) {
        val systemPackages = setOf(ANDROID_PACKAGE, SYSTEM_PACKAGE)
        val systemProcesses = setOf(ANDROID_PACKAGE, SYSTEM_PACKAGE, SYSTEM_SERVER_PROCESS)
        if (lpparam.packageName !in systemPackages || lpparam.processName !in systemProcesses) return
    ...
    companion object {
        ...
        const val SYSTEM_PACKAGE = "system"
