...
object MatrixE2eeUtils {
    ...
    // The SDK's high-level Client API (room.sendRaw()) handles encryption automatically,
     ...
     * 3. Attempt E2EE send via matrix-rust-sdk Client (handles Olm/Megolm automatically)
    ...
    suspend fun sendMsg(context: Context, setting: MatrixSetting, msgInfo: MsgInfo) {
        ...
        val roomEncrypted = try {
            RoomCryptoState.isRoomEncrypted(
                ...
                accessToken = safeSetting.accessToken,
                roomId = safeSetting.roomId,
            )
        } catch (e: Exception) {
            SLog.w(TAG, "Room crypto state query failed, treating as unencrypted: ${e.message}", e)
            false
        }
    ...
    /**
     * Performs the actual encrypted send via matrix-rust-sdk Client.
     * Throws on any failure so the caller can handle fallback.
     *
     * Retry logic: if `room.sendRaw()` fails, retry once with the same parameters.
    ...
    private suspend fun sendEncrypted(
        ...
        // The SDK's Client handles encryption transparently via room.sendRaw(),
        ...
        val contentJson = buildJsonObject {
            ...
            if (messageType == SENDER_MESSAGE_TYPE_MARKDOWN) {
                ...
                put("formatted_body", MatrixUtils.buildFormattedBody(title, msgInfo.content))
            }
        }.toString()
        ...
        // Attempt 1: try room.sendRaw()
        try {
            room.sendRaw(
                eventType = "m.room.message",
                content = contentJson,
            )
            SLog.i(TAG, "Matrix E2EE send success")
            return@withContext
        } catch (firstAttemptError: Exception) {
            val reason = categorizeFailureReason(firstAttemptError)
            SLog.w(
                TAG,
                "E2EE sendRaw failed (attempt=1, reason=$reason, roomId=${setting.roomId}): " +
                    "${firstAttemptError.javaClass.simpleName}: ${firstAttemptError.message}",
            )

            // Attempt 2: retry room.sendRaw() once
            try {
                room.sendRaw(
                    eventType = "m.room.message",
                    content = contentJson,
                )
                SLog.i(TAG, "Matrix E2EE send success on retry (attempt=2)")
            } catch (retryError: Exception) {
                val retryReason = categorizeFailureReason(retryError)
                SLog.w(
                    TAG,
                    "E2EE sendRaw retry failed (attempt=2, reason=$retryReason, roomId=${setting.roomId}): " +
                        "${retryError.javaClass.simpleName}: ${retryError.message}",
                )
                // Propagate to caller for plaintext fallback
                throw retryError
            }
        }
    }
     ...
     *
     * The matrix-rust-sdk high-level Client API (room.sendRaw()) automatically handles:
    ...
    private suspend fun ensureKeySyncReady(client: Client) {
        ...
        try {
            // Wait for background E2EE initialization tasks (key generation, etc.)
            // that were started by restoreSession()
            ...
            // Perform a single non-blocking sync (timeout=0) to:
            // 1. Upload device keys if not yet uploaded
            // 2. Process any pending to-device events (key shares, session setup)
            // 3. Allow the server to report one-time key counts for replenishment
            ...
            // Sync failure is non-fatal: proceed with encryption using existing state.
            // The SDK may still be able to encrypt if keys were previously uploaded.
            // On the next send attempt, we'll try the sync again.
            SLog.w(
                TAG,
                "Initial key sync failed (non-fatal, will retry next send): " +
                    "${e.javaClass.simpleName}: ${e.message}",
                e,
            )
        }
    ...
    private suspend fun getOrCreateClient(context: Context, setting: MatrixSetting): Client {
        ...
        clientMutex.withLock {
            // Return cached client if the access token hasn't changed
            cachedClient?.let { client ->
                if (cachedAccessToken == token) return client
            ...
            try {
                return withTimeout(CLIENT_INIT_TIMEOUT_MS) {
    ...
    private suspend fun buildAndRestoreClient(context: Context, setting: MatrixSetting, token: String): Client {
        // Resolve userId and deviceId via /whoami (with caching)
        val whoami = resolveWhoami(context, setting)
        val storeDir = getStoreDir(context, whoami.userId)

        // Cache whoami result to store directory for future use
        saveWhoamiCache(storeDir, whoami, token)

        return try {
            buildClientWithSession(setting, storeDir, whoami, token)
        } catch (e: Exception) {
            if (isStoreCorruptionException(e)) {
                SLog.w(
                    TAG,
                    "Crypto store corrupted, deleting store and retrying. " +
                        "Reason: ${e.javaClass.simpleName}: ${e.message}",
                    e,
                )
                // Delete the corrupted store directory and reinitialize
                storeDir.deleteRecursively()
                // Recreate the store directory
                storeDir.mkdirs()
                // Re-save whoami cache after directory recreation
                saveWhoamiCache(storeDir, whoami, token)
    ...
    private suspend fun buildClientWithSession(
        ...
        val builder = ClientBuilder()
            .homeserverUrl(setting.homeserver.trim().trimEnd('/'))
        ...
        // Restore session with the real userId and deviceId from whoami
        try {
            ...
            // Close the client we just built if session restore fails
            closeClientSafely(client)
            throw e
        }
