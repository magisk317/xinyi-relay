    ...
    override suspend fun dispatchToSender(
        ...
        try {
            when (safeSender.type) {
                ...
                SenderType.BARK -> BarkUtils.sendMsg(
                    SenderSettingJson.decode(BarkSetting.serializer(), safeSender.jsonSetting),
                    msgInfo,
                )
                SenderType.WEBHOOK -> WebhookUtils.sendMsg(
                    SenderSettingJson.decode(WebhookSetting.serializer(), safeSender.jsonSetting),
                    msgInfo,
                    traceId,
                )
                SenderType.WEWORK_ROBOT -> WeworkRobotUtils.sendMsg(
                    SenderSettingJson.decode(WeworkRobotSetting.serializer(), safeSender.jsonSetting),
                    msgInfo,
                )
                SenderType.WEWORK_AGENT -> WeworkAgentUtils.sendMsg(
                    SenderSettingJson.decode(WeworkAgentSetting.serializer(), safeSender.jsonSetting),
                    msgInfo,
                )
                SenderType.SERVERCHAN -> ServerchanUtils.sendMsg(
                    SenderSettingJson.decode(ServerchanSetting.serializer(), safeSender.jsonSetting),
                    msgInfo,
                )
                SenderType.PUSHPLUS -> PushplusUtils.sendMsg(
                    SenderSettingJson.decode(PushplusSetting.serializer(), safeSender.jsonSetting),
                    msgInfo,
                )
                SenderType.TELEGRAM -> TelegramUtils.sendMsg(
                    SenderSettingJson.decode(TelegramSetting.serializer(), safeSender.jsonSetting),
                    msgInfo,
                )
                SenderType.SMS -> {
                    if (!BuildConfig.ENABLE_SMS_CHANNEL) {
                        SLog.w(
                            "DefaultSenderDispatcher",
                            "SMS sender disabled in current distribution, skipping sender [${safeSender.name}]",
                        )
                    } else {
                        SmsUtils.sendMsg(
                            context,
                            SenderSettingJson.decode(SmsSetting.serializer(), safeSender.jsonSetting),
                            msgInfo,
                        )
                    }
                }
                SenderType.FEISHU -> FeishuUtils.sendMsg(
                    SenderSettingJson.decode(FeishuSetting.serializer(), safeSender.jsonSetting),
                    msgInfo,
                )
                SenderType.GOTIFY -> GotifyUtils.sendMsg(
                    SenderSettingJson.decode(GotifySetting.serializer(), safeSender.jsonSetting),
                    msgInfo,
                )
                SenderType.NTFY -> NtfyUtils.sendMsg(
                    SenderSettingJson.decode(NtfySetting.serializer(), safeSender.jsonSetting),
                    msgInfo,
                )
                ...
                SenderType.FEISHU_APP -> FeishuAppUtils.sendMsg(
                    SenderSettingJson.decode(FeishuAppSetting.serializer(), safeSender.jsonSetting),
                    msgInfo,
                )
                SenderType.URL_SCHEME -> UrlSchemeUtils.sendMsg(
                    context,
                    SenderSettingJson.decode(UrlSchemeSetting.serializer(), safeSender.jsonSetting),
                    msgInfo,
                )
