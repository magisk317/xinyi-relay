...
class StorageRuntimeGraph private constructor(
    context: Context,
) {
    ...
    companion object {
        @Volatile
        private var instance: StorageRuntimeGraph? = null

        fun from(context: Context): StorageRuntimeGraph {
            val existing = instance
            if (existing != null) return existing
            return synchronized(this) {
                instance ?: StorageRuntimeGraph(context).also { instance = it }
            }
        }
    }
}
