...
/**
 * Expressive/Material chrome for the forward-filter rule screen of the
 * same name (shared by the sender-scoped and global rule screens):
 * static top bar with an add `TextButton` action; the dismissible
 * snackbar host stays on the scaffold slot. The glass top bar samples
 * the page content recorded by the inner `Box`.
 */
@Composable
