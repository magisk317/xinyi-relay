...
fun filterNonNegativeIntegerInput(raw: String): String = buildString(raw.length) {
    raw.forEach { ch ->
        ch.digitToIntOrNull()?.let { append(it) }
    }
}

fun normalizeIntegerInput(raw: String): String {
...
fun parseNonNegativeLongInput(raw: String): Long? {
    return normalizeIntegerInput(raw)
        .toLongOrNull()
        ?.takeIf { it >= 0L }
}

fun parseIntAtLeastInput(raw: String, min: Int): Int? {
    return normalizeIntegerInput(raw)
        .toIntOrNull()
        ?.takeIf { it >= min }
}

fun parseIntInRangeInput(raw: String, range: IntRange): Int? {
    return normalizeIntegerInput(raw)
        .toIntOrNull()
        ?.takeIf { it in range }
}
