...
/**
 * Miuix chrome for the remote-agent binding screen of the same
 * name: static top bar hosted in a page-owned miuix `Scaffold` with
 * horizontal-only content window insets; the dismissible snackbar
 * host stays on the scaffold slot (normalized with navigation-bar
 * padding). The glass top bar samples the page content recorded by
 * the inner `Box`.
 */
@Composable
internal fun RemoteAgentScreenMiuix(
    title: String,
    onBack: () -> Unit,
