...
/**
 * Miuix chrome for the relay-config hub screen of the same name:
 * static top bar hosted in a page-owned miuix `Scaffold` with
 * horizontal-only content window insets; the dismissible snackbar
 * host stays on the scaffold slot. The dedup-window dialog tail
 * stays at entry level.
 */
@Composable
internal fun RelayConfigScreenMiuix(
    title: String,
    onBack: () -> Unit,
