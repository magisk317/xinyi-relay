...
/**
 * Expressive/Material chrome for the relay-config hub screen of
 * the same name: static top bar hosted in a page-owned `Scaffold`;
 * the dismissible snackbar host stays on the scaffold slot. The
 * glass top bar samples the page content recorded by the inner
 * `Box`. The dedup-window dialog tail stays at entry level.
 */
@Composable
internal fun RelayConfigScreenMaterial(
    title: String,
    onBack: () -> Unit,
