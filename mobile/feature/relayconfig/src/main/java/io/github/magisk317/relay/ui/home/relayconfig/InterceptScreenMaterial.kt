 ...
 * screen of the same name: static top bar WITHOUT a navigation icon
 * (the page is a nav-host tab) keeping the page-specific content
 * window insets (safeDrawing horizontal + bottom) so the body `Box`
 * that consumes them behaves exactly as before; the body receives
 * the full inner padding. The dismissible snackbar host stays on
 * the scaffold slot. The dialog tail stays at entry level.
 */
@Composable
