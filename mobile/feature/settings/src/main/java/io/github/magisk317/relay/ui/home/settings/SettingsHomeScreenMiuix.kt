 ...
 * hosted in a page-owned miuix `Scaffold` with horizontal-only content
 * window insets; the glass top bar samples the page content recorded by
 * the inner `Box`. The back icon renders only when the entry supplies an
 * `onBack`; the snackbar and the chooser dialog stay at entry level.
 */
@Composable
internal fun SettingsHomeScreenMiuix(
    title: String,
    onBack: (() -> Unit)?,
