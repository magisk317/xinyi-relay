...
/**
 * Expressive/Material chrome for [BlacklistHitListScreen]: static M3 top bar
 * hosted in a page-owned `Scaffold`, with the glass top bar sampling the page
 * content recorded by the inner `Box` (same glass model as
 * OverviewScreenMaterial). The list receives its top inset from the scaffold
 * so content never hides under the bar.
 */
@Composable
internal fun BlacklistHitListScreenMaterial(
    title: String,
    onBack: () -> Unit,
    onOpenClear: () -> Unit,
    onOpenSettings: () -> Unit,
