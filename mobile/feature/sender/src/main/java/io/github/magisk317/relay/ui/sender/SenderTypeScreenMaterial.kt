...
/**
 * Expressive/Material chrome for the sender-type picker of the same
 * name: static top bar hosted in a page-owned `Scaffold`; the glass
 * top bar samples the page content recorded by the inner `Box`. The
 * body receives its top inset from the scaffold inner padding so
 * content never hides under the bar.
 */
@Composable
internal fun SenderTypeScreenMaterial(
    title: String,
    onBack: () -> Unit,
