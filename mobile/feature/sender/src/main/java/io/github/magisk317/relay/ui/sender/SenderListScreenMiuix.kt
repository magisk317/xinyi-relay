 ...
 * top bar hosted in a page-owned miuix `Scaffold` with an add `FAB`
 * slot and horizontal-only content window insets; the glass top bar
 * samples the page content recorded by the inner `Box`. Dialogs, the
 * drag-reorder helper and the snackbar host stay at entry level.
 */
@Composable
internal fun SenderListScreenMiuix(
    title: String,
    onAddClick: () -> Unit,
