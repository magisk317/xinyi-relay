...
/**
 * Expressive/Material chrome for the cloud-backup screen of the
 * same name: static top bar hosted in a page-owned `Scaffold`
 * without a snackbar slot (the page shows messages through the
 *  ambient `LocalSnackbarHostState`). The glass top bar samples the
 * page content recorded by the inner `Box`.
 */
@Composable
internal fun CloudBackupScreenMaterial(
    title: String,
    onBack: () -> Unit,
