...
/**
 * Expressive/Material chrome for the rule screen of the same name:
 * static top bar hosted in a page-owned `Scaffold`; the glass top bar samples the page
 * content recorded by the inner `Box`. The body receives its top inset from the scaffold
 * inner padding so content never hides under the bar.
 */
@Composable
                    ...
                    actions = {
                        AppIconButton(onClick = onSourceSettingsClick) {
                            AppIcon(
                                ...
                                contentDescription = stringResource(R.string.action_rule_source_settings),
                            )
                        }
                        AppIconButton(onClick = onRefresh, enabled = refreshEnabled) {
                            AppIcon(
