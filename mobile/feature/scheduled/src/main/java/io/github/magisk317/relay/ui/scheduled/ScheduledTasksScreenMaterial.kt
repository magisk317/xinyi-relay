...
/**
 * Expressive/Material chrome for the scheduled-task list screen of
 * the same name: static top bar hosted in a page-owned `Scaffold`
 * with an add `FAB` slot; the glass top bar samples the page content
 * recorded by the inner `Box`. Dialogs and the error snackbar live
 * inside the body lambda (they were part of the scaffold content).
 */
@Composable
internal fun ScheduledTasksScreenMaterial(
    title: String,
    onBack: () -> Unit,
    onAddClick: () -> Unit,
