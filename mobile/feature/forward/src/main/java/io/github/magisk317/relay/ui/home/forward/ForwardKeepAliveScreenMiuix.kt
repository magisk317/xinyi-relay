...
/**
 * Miuix chrome for the background-keepalive settings screen of
 * the same name: static top bar hosted in a page-owned miuix
 * `Scaffold` with horizontal-only content window insets; the
 * dismissible snackbar host stays on the scaffold slot. The
 * settings dialog tail stays at entry level.
 */
@Composable
internal fun ForwardKeepAliveScreenMiuix(
    title: String,
    onBack: () -> Unit,
