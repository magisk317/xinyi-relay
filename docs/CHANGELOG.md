...
- 概览卡片编辑模式优化：顺/逆时针晃动、拖动排序与删除操作更顺滑。
- 仅支持 libxposed API 101；legacy 框架（API <= 100）不再兼容，请停留旧版本。
- 验证码设置调整：拦截验证码短信移至实验性功能。
- 远端偏好同步可靠性提升：离线标记待同步，服务恢复后自动补同步。
- 部分页面图标与复制/可见性图标调整，语义更一致。
