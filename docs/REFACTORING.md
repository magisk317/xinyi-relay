...
- 当前仍直接依赖 `runtime` / `relay/android` 的 prefs 和 dispatch DTO，这是兼容期债务，不应继续扩大。
...
- `xpbridge/core` 仍直接依赖 `runtime` / `relay/android` 的 prefs 和 dispatch DTO，应继续拆成纯 facade + 平台实现；notification、SMS runtime、clipboard、app config、record 与 diagnostics bridge 已完成合同/平台适配拆分。
